<template>
  <div class="dashboard selling-page">
          ...
          <div class="card">
            <div class="row">
              <div class="col-md-12">
                <div class="flex">
                  <p class="flex">
                    <span class="buyers-title">Sellers</span>
                  </p>
              ...
              <div class="row">
                <div class="col-md-12 table-container" data-simplebar data-simplebar-auto-hide="false">
                  <table class="col-md-12">
                    <tr v-for="(seller, key) in sellers">
                      <td width="40px" style="padding: 0 10px;">
                        <label class="custom-checkbox">
                          <input type="checkbox" v-on:click="getChecked(seller.user.uid, $event); getExportArray(seller.uid, $event)">
                          <span class="checkmark"></span>
                        </label>
                      </td>
                      <td width="75px">
                        <p :data-letters="seller.user.first_name[0]" :class="'bg_'+key" class="buyer-avatar ml-6"></p>
                      </td>
                      <td width="160px">
                        <span class="card-name user-name">{{ seller.user.first_name }} {{ seller.user.last_name }}</span>
                        <p class="card-doc flex mt-3 user-edit" style="width: 100px;" @click="showEditModal(seller.uid)">
                          <span class="card-contacts-a text-uppercase mont pointer"><i class="fa fa-pen"></i> Edit</span>
                        </p>
                      </td>
                      <td width="255px">
                        <p class="card-doc flex mt-3" @click="showStatusModal(seller.uid)">
                          <span class="pulse-outside">
                            <span class="pulse"></span>
                          </span>
                          <span class="milestones-text">{{ seller.milestones }}</span>
                          <i class="fas fa-pen doc-pen pointer"></i>
                        </p>
                      </td>
                      <td>
                      <span class="faves-qty mt-3">
                        {{ seller.customer_favorite_count }} <img src="../../public/img/arrow-up.svg" class="number-up pointer" alt>
                      </span>
                      </td>
                      <td width="320px">
                        <span class="card-contacts-a text-uppercase mont pointer" @click="showEditModal(seller.uid)">
                          <i class="fa fa-pen"></i> Edit</span>
                        <span class="card-contacts-a text-uppercase mont pointer" @click="exportData(seller.uid)">
                          <img src="../../public/img/export-icon.svg" class="buyer-icons pointer" alt>Export</span>
                        <span class="card-contacts-a text-uppercase mont pointer" @click="showContactModal(seller.uid)">
  ...
  export default {
    ...
    methods: {
      ...
      getSellers() {
        ...
        let url = `/api/realtor/dashboard/home-buying/?seller=1&limit=10&offset=${offSet}`;
      ...
      showStatusModal(uid) {
        ...
        this.selectedUser = (this.sellers).find(obj => {
          return obj.uid === uid
        });
        this.selectedUser.requirements = this.selectedUser.properties;
      },
        ...
        if (this.checkedArray.length > 0 || uid) {
          ...
          if (uid) {
            ...
            $.each(this.checkedArray, function (index, value) {
              self.selectedUsers.push((self.sellers).find(obj => {
          ...
          this.showLoading = false;
          this.$toastr("warning", "Please choose sellers to contact", "Warning!");
        }
      },
      showEditModal(uid) {
      ...
      exportData(uid) {
        ...
        if (this.exportArray.length > 0 || uid) {
          axios({
            url: `https://staging.homecaptain.com/api/realtor/dashboard/home-buying/export-customers/?seller=1&uids=${data.join()}`,
            method: 'GET',
            responseType: 'blob',
          }).then((response) => {
            ...
            link.href = url;
            link.setAttribute('download', 'Realtor-Sellers-Export.xlsx');
          ...
          this.showLoading = false;
          this.$toastr("warning", "Please choose sellers to export", "Warning!");
        }
      },
      getStats() {
        axios
          .get("/api/realtor/dashboard/home-buying/stats/?seller=1")
          .then((res) => {
